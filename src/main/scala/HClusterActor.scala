  ...
  class HClusterActor extends Actor {
    ...
    def receive = {
      case clusterData: hClusterData => {
        println("updating with " + clusterData.currentClustID)
        ...
        clusters.zipWithIndex.foreach { case (x, i) =>
          if ( i < clusters.length-1) {
            println("drop size " + clusters.drop(i+1).length)
      ...
      case results: ComputeResults => {
        //println("cluster count " + computingClusterCounter)
        //println(results)
        if (computingClusterCounter != 0) {
          ...
          //println(results.clAndLw.closest, ForAllClAndLw.closest)
          ...
          currentClustID -= 1
          println("was size " + clusters.length)
          ...
          println("now size " + clusters.length)
          //println("remaining after drop " + clusters.length )
