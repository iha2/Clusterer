...
case class Vec(cells: Vector[Double]) {
  ...
  def sum(that: Vec): Vec = {

    if (cells.length != that.cells.length) {
      throw new Exception("Lengths of vectors are not the same")
    } else {
      new Vec(cells.zip(that.cells).map( x => x._1 + x._2))
    }
  }
}
case class Cluster(vec: Vec, left: Cluster, right: Cluster, distance: Double, id: String) {

  def id(x: String) = this.copy(this.vec, this.left, this.right, this.distance, x)

  def merge(that: Cluster): Cluster = {
    val newVec = this.vec.merge(this.vec)
    this.copy(newVec, this.left, this.right, this.distance, "")
  }
}
